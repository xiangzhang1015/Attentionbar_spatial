from env_spatial_hmm import Env
import pickle
    ...
    for i in range(MAX_EPISODES):
        ...
        for step in range(MAX_EP_STEPS):
            ...
            if current_reward > best_reward:
                best_reward = reward
                best_state = [acc, reward, state, state_]
...
if __name__ == "__main__":
    ...
    len_max = 128
    env = Env(len_max=len_max, n_fe=14, n_classes=8)
    ...
    # env.after(10, run_env)
    best_reward, best_state, reward_his = run_env()
    print best_state, best_reward
    # env.mainloop()
    # RL.plot_cost()
    pickle.dump(RL.cost_his, open("cost_his", "wb"))
    pickle.dump(reward_his, open("reward_his", "wb"))
